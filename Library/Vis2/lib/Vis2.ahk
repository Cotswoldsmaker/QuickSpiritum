...
class Vis2 {
   ...
   class OCR extends Vis2.functor {
      call(self, image:="", language:="", options:=""){
         return (image != "") ? (new Vis2.provider.Tesseract()).OCR(image, language, options)
            : Vis2.core.returnText({"provider":(new Vis2.provider.Tesseract(language)), "tooltip":"Click and drag around an MRN on screen. If for manual enter, just double click anywhere on the screen", "textPreview":true})
      }
   ...
   class provider {
      ...
      class Tesseract {
         ...
         static leptonica := A_ScriptDir "\library\vis2\bin\leptonica_util\leptonica_util.exe"
         static tesseract := A_ScriptDir "\library\vis2\bin\tesseract\tesseract.exe"
         static tessdata_best := A_ScriptDir "\library\vis2\bin\tesseract\tessdata_best"
         static tessdata_fast := A_ScriptDir "\library\vis2\bin\tesseract\tessdata_fast"
